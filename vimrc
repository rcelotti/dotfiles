...
map <F8> :make<CR>
" build using makeprg with <S-F8>
map <S-F8> :make clean all<CR>
...
map <C-M-j> /^[^[:blank:]]\+.\+{<CR>:noh<CR>
map <C-M-k> ?^[^[:blank:]]\+.\+{<CR>:noh<CR>
...
" let NERDTreeIgnore=['\.pyc',pyc '\~$', '\.swo$', '\.swp$', '\.git', '\.hg', '\.svn', '\.bzr']
